...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("unitedscifi-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #unitedscifiTEST2\r");
            Send(hSocket, "WHO #unitedscifiTEST2\r");
        } else {
            // randomly join #unitedscifi00-#unitedscifi05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #unitedscifi%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #unitedscifi%02d\r", channel_number).c_str());
        }
