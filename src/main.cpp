...
CBigNum bnProofOfWorkLimit(~uint256(0) >> 24); // target limit for proof of work, results with 0,00390625 proof-of-work difficulty
CBigNum bnProofOfStakeLegacyLimit(~uint256(0) >> 24); // proof of stake target limit with 0,00390625 proof of stake difficulty
CBigNum bnProofOfStakeLimit(~uint256(0) >> 27); // 27 proof of stake target limit equal to 0.03125  proof of stake difficulty
...
uint256 nPoWBase = uint256("0x000000ffff000000000000000000000000000000000000000000000000000000"); // difficulty-1 target

CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 20);

unsigned int nStakeMinAge = 10 * nOneDay; // 10 days as zero time weight
unsigned int nStakeMaxAge = 30 * nOneDay; // 30 days as full weight
unsigned int nStakeTargetSpacing = 3 * 60; // 3-minute stakes spacing
unsigned int nModifierInterval = 3 * 60 * 60; // nOneHour; // time to elapse before new modifier is computed
...
const string strMessageMagic = "UnitedSciFi Signed Message:\n";
...
// miner's coin stake reward based on nTime
...
static const int64_t nTargetTimespan = 100 * nStakeTargetSpacing;  // 100 blocks
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("unitedscifi-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in UnitedSciFi
    ...
    if (IsProofOfWork() && pindex->nHeight > 1)
    {
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // UnitedSciFi: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    int nMaxOffset = 48 * nOneHour; // 48 hours
    if (fTestNet || pindexPrev->nTime < 1512086400)
        nMaxOffset = 7 * nOneWeek; // 7 week (permanently on testNet or until 01 Dec 2017 on mainNet)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "UnitedSciFi", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeTargetSpacing = 0.5 * 60; // test block spacing is 0.5 minutes
    }
  ...
  vMerkleTree: 15461d8c64
        */
        ...
        {
            for(block.nNonce = 0; block.GetHash() > bnProofOfWorkLimit.getuint256(); block.nNonce++){ } 
            block.print();
        }
...
unsigned char pchMessageStart[4] = { 0xc6, 0xe7, 0xef, 0xfd };
