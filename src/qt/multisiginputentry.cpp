...
void MultisigInputEntry::on_transactionId_textChanged(const QString &transactionId)
{
    ...
    for(unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        if(ExtractDestination(script, addr))
        {
            ...
            ui->transactionOutput->addItem(idStr + QString(" - ") + addressStr + QString(" - ") + amountStr + QString(" SCIFI"));
        }
        else
            ui->transactionOutput->addItem(idStr + QString(" - ") + amountStr + QString(" SCIFI"));
    }
}
