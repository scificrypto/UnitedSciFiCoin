...
*/
    return (uint64_t)GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nHeight);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (uint64_t)GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nHeight)));
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "UnitedSciFi is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "UnitedSciFi is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "UnitedSciFi is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "UnitedSciFi is downloading blocks...");
