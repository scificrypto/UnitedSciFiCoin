...
const std::string CLIENT_NAME("UnitedSciFi");
...
#    define GIT_COMMIT_DATE "13 June 2020"
#endif

#define BUILD_DESC_FROM_COMMIT(maj,min,rev,commit) \
    "scifi-v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(rev) "." DO_STRINGIZE(build) "-g" commit

#define BUILD_DESC_FROM_UNKNOWN(maj,min,rev) \
    "scifi-v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(rev) "." DO_STRINGIZE(build) "-unk"
