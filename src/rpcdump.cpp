...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <unitedscifiprivkey> [label] [rescan=true]\n"
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid UnitedSciFi address or script");
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <unitedscifiaddress>\n"
            "Reveals the private key corresponding to <unitedscifiaddress>.");
...
Value dumppem(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "dumppem <unitedscifiaddress> <filename> <passphrase>\n"
            "Dump the key pair corresponding to <unitedscifiaddress> and store it as encrypted PEM file."
            + HelpRequiringPassphrase());
