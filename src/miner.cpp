...
bool FillMap(CWallet *pwallet, uint32_t nUpperTime, MidstateMap &inputsMap)
{
    ...
    CTxDB txdb("r");
    {
        ...
        CoinsSet setCoins;
        int64_t nValueIn = 0;
        if (!pwallet->SelectCoinsSimple(nBalance - nReserveBalance, MIN_TX_FEE, MAX_MONEY, nUpperTime, nCoinbaseMaturity * 72, setCoins, nValueIn))
...
void ThreadStakeMiner(void* parg)
{
    ...
    RenameThread("unitedscifi-miner");
